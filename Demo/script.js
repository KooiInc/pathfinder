...
// testData is from json.
// serialized JSON can not contain undefined values, so set it first for demonstration
...
const testDataWithChangedYellowPathValue = yellowPath.cloneWithNewValue("RED");

log(`\n<b>Cloned testData with new value for key 'yellow' (declaration =>
  ${toCodeCmd(`const testDataWithChangedYellowPathValue = yellowPath.cloneWithNewValue("RED")`)})</b>`);
...
document.querySelector(`details > pre > code`).textContent = JSON.stringify(testData, null, 2);
document.querySelector(`.demo`).append(document.querySelector(`#report`));
