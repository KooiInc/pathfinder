const addMethod2Prototype = await doImport("https://kooiinc.github.io/ProtoXT/protoxt.js");
...
const createReturnValue = (exists, value = noValue) => ({exists, value});
...
const extTo = (me, key) => validObj(me) && findPathForKey(me, key) || { searchKey: key, pathFound: invObj, };
const extFrom = (me, path) => validObj(me) && getValueFromPath(me, path) || { exists: false[yn], value: invObj, };
...
/**
 * can [path] be found within [obj]?
 * To include paths from arrays (containing objecs)
 * use [includeArrays]
...
/**
 * find the path of a given key within [obj]
 * using getPathFromObject to retrieve the value if applicable
 */
function findPathForKey(initialObj, key) {
  let result = {
    searchKey: key, // save original search key
    pathFound: false[yn],
  };
      ...
      if (k === key) {
        ...
        result.cloneWithNewValue = value => {
          const cloned = window.structuredClone 
            ? structuredClone(initialObj) : JSON.parse(JSON.stringify(initialObj));
